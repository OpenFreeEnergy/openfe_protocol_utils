...
def assign_offmol_espaloma_charges(
    offmol: OFFMol, toolkit_registry: ToolkitRegistry
) -> None:
    """
    ...
    if not HAS_ESPALOMA:
        errmsg = (
            "The Espaloma ToolkiWrapper is not available, "
            "please install espaloma_charge"
        )
        raise ImportError(errmsg)

    warnings.warn(
        "Using espaloma to assign charges is not well tested", category=RuntimeWarning
    )
        ...
        with ProcessPoolExecutor(max_workers=processors) as pool:
            ...
            for work in tqdm.tqdm(
                ...
                charged_ligands.append(
                    SmallMoleculeComponent.from_openff(work.result())
                )

    else:
        for m in tqdm.tqdm(
            molecules, desc="Generating charges", ncols=80, total=len(molecules)
        ):
