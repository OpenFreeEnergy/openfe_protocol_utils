...
from openfe_protocol_utils.settings import (
    omm_settings,
    settings_validation,
)
from openfe_protocol_utils.analysis import (
    multistate_analysis,
    plotting,
)
from openfe_protocol_utils.system_validation import system_validation
from openfe_protocol_utils.system_creation import (
    system_creation,
    charge_generation,
)
from openfe_protocol_utils.system_creation.charge_generation import (
    HAS_NAGL,
    HAS_ESPALOMA,
    HAS_OPENEYE,
)
...
@pytest.mark.parametrize(
    ...
    [
        [
            1.2 * unit.nanometer,
            20,
            20 * np.identity(3) * unit.angstrom,
            [2, 2, 2] * unit.angstrom,
        ],
...
def test_validate_ommsolvation_settings_unique_settings(
    ...
    settings = omm_settings.OpenMMSolvationSettings(
...
def test_get_alchemical_components(benzene_modifications, T4_protein_component):
    ...
    stateA = gufe.ChemicalSystem(
        {
            ...
            "S": gufe.SolventComponent(smiles="C"),
        }
    )
    ...
    assert gufe.SolventComponent(smiles="C") in alchem_comps["stateA"]
    assert len(alchem_comps["stateB"]) == 2
    assert benzene_modifications["benzonitrile"] in alchem_comps["stateB"]
    assert gufe.SolventComponent() in alchem_comps["stateB"]
...
def test_duplicate_chemical_components(benzene_modifications):
    ...
    stateB = gufe.ChemicalSystem({"A": benzene_modifications["toluene"]})
...
def test_validate_solvent_nocutoff(benzene_modifications):
    ...
    state = gufe.ChemicalSystem(
        {"A": benzene_modifications["toluene"], "S": gufe.SolventComponent()}
    )
...
def test_not_water_solvent(benzene_modifications):
    ...
    state = gufe.ChemicalSystem(
        {"A": benzene_modifications["toluene"], "S": gufe.SolventComponent(smiles="C")}
    )
...
def test_multiple_proteins(T4_protein_component):
    ...
    state = gufe.ChemicalSystem({"A": T4_protein_component, "B": T4_protein_component})
...
def test_components_solvent(benzene_modifications):
    ...
    assert s == gufe.SolventComponent()
    assert p is None
    assert len(mols) == 2
...
def get_settings():
    ...
    integrator_settings = omm_settings.IntegratorSettings()
...
class TestFEAnalysis:
    ...
    # Note: class scope _may_ cause this to segfault - the reporter has to close
    # Having analyzer at class scope seems to help here
    @pytest.fixture(scope="class")
    def reporter(self):
        with resources.files("openfe_protocol_utils.tests.data.openmm_rfe") as d:
...
class TestSystemCreation:
    ...
    def test_get_omm_modeller_complex(
        self, T4_protein_component, benzene_modifications, get_settings
    ):
        ffsets, intsets, thermosets = get_settings
        generator = system_creation.get_system_generator(
        ...
        model, comp_resids = system_creation.get_omm_modeller(
            ...
            gufe.SolventComponent(),
            {smc: mol},
            generator.forcefield,
            omm_settings.OpenMMSolvationSettings(),
        )
        ...
        assert_equal(
            comp_resids[gufe.SolventComponent()],
    ...
    def ligand_mol_and_generator(self, get_settings):
        ...
        smc = gufe.SmallMoleculeComponent.from_openff(offmol)
    ...
    def test_get_omm_modeller_ligand_no_neutralize(self, ligand_mol_and_generator):
        ...
        model, comp_resids = system_creation.get_omm_modeller(
            None,
            gufe.SolventComponent(neutralize=False),
            {smc: offmol},
            generator.forcefield,
            omm_settings.OpenMMSolvationSettings(),
        )
    ...
    def test_omm_modeller_ligand_n_solv(
        ...
        solv_settings = omm_settings.OpenMMSolvationSettings(
            solvent_padding=None,
        ...
        model, comp_resids = system_creation.get_omm_modeller(
            None,
            gufe.SolventComponent(
                neutralize=neutralize, ion_concentration=0 * unit.molar
            ),
            {smc: offmol},
            generator.forcefield,
            solv_settings,
    ...
    def test_omm_modeller_box_size(self, ligand_mol_and_generator):
        ...
        assert_allclose(
            from_openmm(vectors), [[2, 0, 0], [0, 2, 0], [0, 0, 2]] * unit.nanometer
        )
    ...
    def test_omm_modeller_box_vectors(self, ligand_mol_and_generator):
        ...
        assert_allclose(
            from_openmm(vectors), [[2, 0, 0], [0, 2, 0], [0, 0, 5]] * unit.nanometer
        )
...
class TestOFFPartialCharge:
    ...
    def test_am1bcc_reference(
        ...
        assert_allclose(
            am1bcc_ref_charges[ref_key], uncharged_mol.partial_charges, rtol=1e-4
        )

    def test_nagl_import_error(self, monkeypatch, uncharged_mol):
        monkeypatch.setattr(
            sys.modules["openfe_protocols_utils.system_creation.charge_generation"],
            "HAS_NAGL",
            False,
        )
    ...
    def test_espaloma_import_error(self, monkeypatch, uncharged_mol):
        monkeypatch.setattr(
            sys.modules["openfe_protocol_utils.system_creation.charge_generation"],
            "HAS_ESPALOMA",
            False,
        )
...
def test_forward_backwards_failure(simulation_nc):
    ...
    with mock.patch(
        "openfe_protocol_utils.analysis.multistate_analysis.MultistateEquilFEAnalysis._get_free_energy",
