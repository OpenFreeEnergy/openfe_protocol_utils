...
def plot_lambda_transition_matrix(matrix: npt.NDArray) -> Axes:
    ...
    if not np.allclose(matrix.sum(axis=0), 1.0) or not np.allclose(
...
def plot_replica_timeseries(
    ...
    if equilibration_iterations is not None:
        ax.axvline(
            x=equilibration_iterations,
            color="grey",
            linestyle="--",
            label="equilibration limit",
        )
...
def plot_2D_rmsd(data: list[list[float]], vmax=5.0) -> plt.Figure:
    ...
    for i, (arr, ax) in enumerate(zip(twod_rmsd_arrs, axes.flatten())):  # type: ignore
    ...
    plt.colorbar(
        axes.flatten()[0].images[0],  # type: ignore
