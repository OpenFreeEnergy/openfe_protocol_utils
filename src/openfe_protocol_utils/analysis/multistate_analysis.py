...
from openfe_protocol_utils.analysis import plotting
from typing import Optional, Union
from openfe_protocol_utils.due import due, Doi


due.cite(
    Doi("10.5281/zenodo.596622"),
    description="OpenMMTools",
    path="openfe_protocol_utils.analysis.multistate_analysis",
    cite_module=True,
)
...
class MultistateEquilFEAnalysis:
    ...
    def _analyze(self, forward_reverse_samples: int):
        ...
        self._replica_states = (
            self.analyzer.reporter.read_replica_thermodynamic_states()
        )
    ...
    def _get_free_energy(
        ...
        * Add standard test against analyzer.get_free_energy()
        ...
        return (
            ensure_quantity(DG, "openff").to(return_units),
    ...
    def get_forward_and_reverse_analysis(
        ...
        try:
            ...
            if not np.all(N_l == N_l[0]):
                errmsg = (
                    "The number of samples is not equivalent across all "
                    f"states {N_l}"
                )
            ...
            chunks = [
                max(int(N_l[0] / num_samples * i), 1) for i in range(1, num_samples + 1)
            ]
    ...
    def unit_results_dict(self):
        ...
        if hasattr(self, "_exchange_matrix"):
            results_dict["replica_exchange_statistics"] = (
