    ...
    * Cannot be defined alongside ``solvent_padding``,
    ...
    def is_positive_distance(cls, v):
        ...
        if not v.is_compatible_with(unit.nanometer):
            raise ValueError(
                "solvent_padding must be in distance units " "(i.e. nanometers)"
            )
        if v < 0:
    ...
    def box_size_properties(cls, v):
        ...
        if v.shape != (3,):
            errmsg = (
                f"box_size must be a 1-D array of length 3 "
...
class IntegratorSettings(SettingsBaseModel):
    ...
    def must_be_positive(cls, v):
        if v <= 0:
            errmsg = (
                "timestep, and constraint_tolerance "
    ...
    def must_be_inverse_time(cls, v):
        if not v.is_compatible_with(1 / unit.picosecond):
            raise ValueError(
                "langevin collision_rate must be in inverse time "
                "(i.e. 1/picoseconds)"
            )
        return v
...
class MultiStateOutputSettings(OutputSettings):
    ...
    """
    positions_write_frequency: Optional[FloatQuantity["picosecond"]] = (
        100 * unit.picosecond
    )
    """
...
class MultiStateSimulationSettings(SimulationSettings):
    ...
    * Make n_replicas optional such that: If ``None`` or greater than the
    ...
    """
    real_time_analysis_interval: Optional[FloatQuantity["picosecond"]] = (
        250 * unit.picosecond
    )
    # todo: Add validators in the protocol
    """
    ...
    def supported_flatness(cls, v):
        ...
        if v.lower() not in supported:
            errmsg = (
                "Only the following sams_flatness_criteria are "
    ...
    def must_be_positive(cls, v):
        if v <= 0:
            errmsg = (
                "n_replicas and steps_per_iteration must be positive "
                f"values, got {v}."
            )
            raise ValueError(errmsg)
        return v
